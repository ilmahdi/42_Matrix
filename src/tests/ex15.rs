...
pub fn run() {
    ...
    assert_eq!(
        result,
        Vector::from([c(10., 10.), c(0., -10.), c(230., 0.)])
    );
    ...
    let angle_cos_result = angle_cos_complex(&u, &v);
    assert_eq!(angle_cos_result.abs(), 0.0); // Cosine ~ 0
    ...
    println!("All complex vector and matrix tests passed.");
}
